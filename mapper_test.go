...
func TestNestedStruct(t *testing.T) {
	...
	v = m.FieldByPath(pv, "asset.title")
	if v.Interface().(string) != post.Asset.Title {
		t.Errorf("Expecting %s, got %s", post.Asset.Title, v.Interface().(string))
	}
	v = m.FieldByPath(pv, "asset.details.active")
...
func TestInlineStruct(t *testing.T) {
	m := NewMapper("db", FieldNameToLower)
...
func logTree(t *testing.T, fi *FieldInfo, level int) {
	t.Helper()
	t.Log("level ", level, "  :", fi.Index, fi.Path)
...
func TestFieldsEmbedded(t *testing.T) {
	...
	trs := m.TraversalsByPath(reflect.TypeOf(pp), []string{"person.name", "name", "title"})
...
func TestTagNameMapping(t *testing.T) {
	...
	m := NewMapper("json", func(fieldName, tag string) (string, []string) {
		fieldName = strings.ToUpper(fieldName)
		name, parts := StdTagFunc(fieldName, tag)
		if len(parts) > 1 {
			parts = parts[:1]
		}
		return name, parts
...
func TestMapping(t *testing.T) {
	...
	for _, key := range []string{"p.id", "p.name", "p.wears_glasses", "weight", "age"} {
		if fi := mapping.GetByPath(key); fi == nil {
			t.Errorf("Expecting to find key %s in mapping but did not.", key)
		}
	}
	...
	type RugbyPlayer struct {
		...
		IsAllBlack   bool `db:"-"`
		SportsPerson `db:"sportsperson"`
	}
	r := RugbyPlayer{12, true, false, s}
	mapping = m.TypeMap(reflect.TypeOf(r))
	for _, key := range []string{"p.id", "p.name", "p.wears_glasses", "weight", "age", "position", "is_intense"} {
		if fi := mapping.GetByPath(key); fi == nil {
			t.Errorf("Expecting to find key %s in mapping but did not.", key)
		}
	}
...
// TestMapperMethodsByPath tests Mapper methods FieldByPath and TraversalsByPath
func TestMapperMethodsByPath(t *testing.T) {
	type C struct {
		C0 string
		C1 int
	}
...
func TestDuplicated(t *testing.T) {
	...
	defer func() {
		r := recover()
		if r.(error).Error() != "duplicated path: reflectx.Person.name, indexs are [0] and [1 0]" {
			t.Errorf("expected %v", r)
		}
	}()

	m.TypeMap(reflect.TypeOf(Person{}))
...
func TestStrToValue(t *testing.T) {
	var v *[]int
	vv := reflect.New(reflect.TypeOf(v)).Elem()
	err := StrToValue("3", vv)
	if err == nil || err.Error() != "reflectx: unexpected type for StrToValue: *[]int" {
		t.Error("reflectx: unexpected type for StrToValue: *[]int")
	}
}
